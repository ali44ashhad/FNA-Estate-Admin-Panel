...
export default function OpsPurchasesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return purchases.filter((p) => {
      const hay = `${p?.id ?? ''} ${p?.inventoryKey ?? ''} ${p?.userId ?? ''} ${p?.projectId ?? ''} ${p?.leadId ?? ''} ${
        p?.visitId ?? ''
  ...
  return (
    <div className="space-y-5">
      <PurchasesToolbar
        ...
        subtitle={
          <>
            Ops view: update <span className="font-semibold text-slate-800">status</span> and track booked purchases.
          </>
        }
        total={meta?.total ?? 0}
        query={query}
