...
export function useEmployees({ q, role, page, limit }) {
  ...
  const refresh = useCallback(
    async (nextPage = page) => {
      ...
      setError('')
      try {
        const { items, meta: apiMeta } = await getEmployees({ q, role, page: nextPage, limit })
        setEmployees(items)
        setMeta(
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError('')
      try {
        const { items, meta: apiMeta } = await getEmployees({ q, role, page, limit })
        if (!alive) return
        setEmployees(items)
        setMeta(
          normalizeMeta(apiMeta, {
